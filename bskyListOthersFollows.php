    ...
    if ($pdsURL == '') {
        ...
        if ($bluesky->hasApiKey()) {
            ...
            if ($tUsr = $bluesky->request('GET', 'app.bsky.actor.getProfile', $args)) {
                // Get all the follows for the target user
                $tDID = $tUsr->did;
                $cursor = '';
                $arrFoll = [];
                do {
                ...
                // Output the list of follows
                echo "<h2>Follows of $TARGET_HANDLE:</h2><ul>";
...
<html>
<head>
    <title>List Follows of Another User</title>
</head>
<body>
    <h1>List Follows of Another User</h1>
    <?php
    require "./app-pw.php";
    ?>
