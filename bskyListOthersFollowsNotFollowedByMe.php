        ...
        if ($bluesky->hasApiKey()) {
            ...
            if ($tUsr = $bluesky->request('GET', 'app.bsky.actor.getProfile', $args)) {
                ...
                do {
                    ...
                    $res = $bluesky->request('GET', 'app.bsky.graph.getFollows', $args);
                    $arrYourFollowings = array_merge($arrYourFollowings, (array)$res->follows);
                    $cursor = $res->cursor ?? '';
                } while ($cursor);

                // Find follows of the target user not followed by you
                $followsNotFollowedByMe = array_diff(array_map(fn($f) => $f->did, $arrFollowings), array_map(fn($f) => $f->did, $arrYourFollowings));

                // Create a new list for follows not followed by you
                $args = [
                    ...
                    'record' => [
                        ...
                        'name' => 'Follows of ' . $TARGET_HANDLE . ' Not Followed by Me ' . date('Y-m-d'),
                        'description' => "List of accounts $TARGET_HANDLE follows that I do not follow.",
                    ],
                ];
                if ($data2 = $bluesky->request('POST', 'com.atproto.repo.createRecord', $args)) {
                    $newListURI = $data2->uri;
                    // Now loop the collection of follows not followed by you into the new list
                    foreach ($followsNotFollowedByMe as $did) {
                        // Add to the list
                        $args = [
...
<html>
<head>
    <title>List Follows of Another User Not Followed by Me</title>
</head>
<body>
    <h1>List Follows of Another User Not Followed by Me</h1>
    <?php
    require "./app-pw.php";
    ?>
